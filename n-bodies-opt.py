...
size = comm.Get_size()
rank = comm.Get_rank()
local_data  = np.zeros((nbbodies // size  ,  6) , dtype = 'f')
if rank == 0:
...
bodies_par_processus = nbbodies // size

for t in range(NBSTEPS):
    force = np.zeros((nbbodies, 2))
    for i in range(bodies_par_processus):
        gl_i = i + rank * bodies_par_processus
        for j in range(gl_i):
            force_j_on_i = interaction(data[gl_i], data[j])
            force[gl_i] += force_j_on_i
            force[j] -= force_j_on_i
    comm.Allreduce(MPI.IN_PLACE, force, op=MPI.SUM)
    for i in range(bodies_par_processus):
        gl_i = i + rank * bodies_par_processus
        data[gl_i] = update(data[gl_i], force[gl_i])
    comm.Allgather(MPI.IN_PLACE, data)
